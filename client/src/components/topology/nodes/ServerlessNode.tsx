...
export default React.memo(function ServerlessNode({ data }: NodeProps) {
  const d = data as any;
  const info = subtypeLabels[d?.resourceType] || { short: 'SLS', color: '#A855F7' };

  if (d?.compact) {
    return (
      <div className="bg-white dark:bg-gray-800 border rounded-lg px-2 py-1 flex items-center gap-1.5" style={{ borderColor: info.color }}>
        <span className="w-4 h-4 rounded text-white text-[8px] flex items-center justify-center font-bold shrink-0" style={{ backgroundColor: info.color }}>{info.short}</span>
        <span className="text-[10px] font-medium truncate max-w-[80px] dark:text-gray-200">{d?.label || 'Serverless'}</span>
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border-2 rounded-lg p-2.5 min-w-[130px] shadow-sm" style={{ borderColor: info.color }}>
      <div className="flex items-center gap-2">
        ...
        <div className="text-xs font-semibold truncate max-w-[110px] dark:text-gray-200">{d?.label || 'Serverless'}</div>
      </div>
      <Handle type="target" position={Position.Top} className="!w-2 !h-2" style={{ background: info.color }} />
      <Handle type="source" position={Position.Bottom} className="!w-2 !h-2" style={{ background: info.color }} />
    </div>
