...
const RESOURCE_TYPE_TO_NODE_TYPE: Record<string, string> = {
  ...
  // New resource types
  'network/public-ip': 'genericNode',
  'storage/mount-target': 'storageNode',
  'compute/instance-pool': 'instanceNode',
  'security/waf-policy': 'securityNode',
  'security/bastion': 'securityNode',
  'security/certificate': 'securityNode',
  'observability/alarm': 'genericNode',
  'observability/notification-topic': 'genericNode',
  'observability/events-rule': 'genericNode',
  'database/nosql-table': 'databaseNode',

  // Compute sub-resources
  'compute/volume-attachment': 'instanceNode',
  'compute/dedicated-vm-host': 'instanceNode',
  'compute/capacity-reservation': 'instanceNode',
  'compute/compute-cluster': 'instanceNode',
  'compute/autoscaling-config': 'instanceNode',

  // Network sub-resources
  'network/vlan': 'subnetNode',
  'network/cpe': 'gatewayNode',
  'network/ipsec-connection': 'gatewayNode',
  'network/cross-connect-group': 'gatewayNode',
  'network/cross-connect': 'gatewayNode',
  'network/virtual-circuit': 'gatewayNode',
  'network/remote-peering-connection': 'gatewayNode',
  'network/private-ip': 'genericNode',
  'network/vtap': 'genericNode',
  'network/capture-filter': 'securityNode',
  'network/byoip-range': 'genericNode',
  'network/public-ip-pool': 'genericNode',
  'network/network-firewall': 'securityNode',
  'network/network-firewall-policy': 'securityNode',
  'network/drg-route-distribution': 'gatewayNode',

  // Database sub-resources
  'database/db-backup': 'storageNode',
  'database/autonomous-db-backup': 'storageNode',
  'database/autonomous-container-database': 'databaseNode',
  'database/database': 'databaseNode',
  'database/pluggable-database': 'databaseNode',
  'database/db-node': 'databaseNode',
  'database/exadata-infrastructure': 'databaseNode',
  'database/cloud-vm-cluster': 'databaseNode',
  'database/cloud-exa-infra': 'databaseNode',
  'database/db-software-image': 'databaseNode',
  'database/db-key-store': 'securityNode',
  'database/maintenance-run': 'genericNode',
  'database/data-guard-association': 'databaseNode',
  'database/redis-cluster': 'databaseNode',
  'database/opensearch-cluster': 'databaseNode',
  'database/psql-db-system': 'databaseNode',
  'database/psql-backup': 'storageNode',

  // Storage sub-resources
  'storage/preauth-request': 'storageNode',
  'storage/lifecycle-policy': 'storageNode',
  'storage/replication-policy': 'storageNode',

  // Security additions
  'security/cloud-guard-target': 'securityNode',
  'security/cloud-guard-detector-recipe': 'securityNode',
  'security/waas-policy': 'securityNode',
  'security/waas-certificate': 'securityNode',
  'security/dr-protection-group': 'securityNode',
  'security/dr-plan': 'securityNode',

  // IAM additions
  'iam/auth-token': 'iamNode',
  'iam/smtp-credential': 'iamNode',
  'iam/network-source': 'iamNode',
  'iam/region-subscription': 'iamNode',
  'iam/tag-namespace': 'iamNode',
  'iam/tag-default': 'iamNode',
  'iam/tag': 'iamNode',

  // Observability additions
  'observability/email-sender': 'genericNode',
};
...
function nodeTypeFor(resourceType: string): string {
  ...
  if (resourceType.startsWith('devops/')) return 'containerNode';
  if (resourceType.startsWith('messaging/')) return 'serverlessNode';
  if (resourceType.startsWith('governance/')) return 'genericNode';
  if (resourceType.startsWith('monitoring/')) return 'genericNode';
...
const NETWORK_VIEW_TYPES = new Set([
  ...
  'network/remote-peering-connection',
  'network/cross-connect',
  'network/cross-connect-group',
...
const INFRA_PRIORITY: Record<string, number> = {
  ...
  // Network sub-resources in NETWORK_VIEW_TYPES
  'network/vlan': 2,
  'network/cpe': 3,
  'network/ipsec-connection': 3,
  'network/virtual-circuit': 3,
  'network/remote-peering-connection': 3,
  'network/cross-connect': 4,
  'network/cross-connect-group': 4,
  'network/network-firewall': 3,
};

// ---------------------------------------------------------------
// Types excluded from compartment topology (plumbing / config noise)
// ---------------------------------------------------------------
const TOPOLOGY_EXCLUDED_TYPES = [
  // Attachment plumbing (1:1 or 1:N with parent resource)
  ...
  // Network config objects (represented by their parent)
  'network/dhcp-options',
  'network/nsg-rule',
  'network/drg-route-table',
  'network/drg-route-rule',
  'network/drg-route-distribution',
  'network/private-ip',
  'network/capture-filter',
  'network/byoip-range',
  'network/public-ip-pool',
  'network/network-firewall-policy',
  'network/vtap',
  // Storage config (policies, not actual data)
  'storage/preauth-request',
  'storage/lifecycle-policy',
  'storage/replication-policy',
  // Database operational
  'database/maintenance-run',
  // IAM credentials (per-user detail, not topology-relevant)
  'iam/api-key',
  ...
  'iam/tag-default',
  'iam/tag',
  // DNS records (individual records, can be thousands)
  'dns/record',
...
async function buildNetworkView(
  ...
  const infraResourcesWithRaw = await prisma.resource.findMany({
  ...
  // Build rawData map from the initial query to avoid a second DB pass
  const rawDataMap = new Map<string, string | null>();
  for (const r of infraResourcesWithRaw) {
    rawDataMap.set(r.id, r.rawData);
  }

  // Strip rawData for the metadata-only array used downstream
  const infraResources: ResourceMeta[] = infraResourcesWithRaw.map(r => ({
    id: r.id, ocid: r.ocid, resourceType: r.resourceType,
    displayName: r.displayName, lifecycleState: r.lifecycleState, compartmentId: r.compartmentId,
  }));
  ...
  // Build lookup maps for O(1) access instead of Array.find()
  const ocidToDbId = new Map<string, string>();
  const cappedById = new Map<string, ResourceMeta>();
  const cappedByOcid = new Map<string, ResourceMeta>();
  for (const r of capped) {
    ocidToDbId.set(r.ocid, r.id);
    cappedById.set(r.id, r);
    cappedByOcid.set(r.ocid, r);
  }

  // Also load rawData for individually-fetched instances (if not collapsed)
  if (!collapseInstances && instances.length > 0) {
    const instanceIds = instances.map(r => r.id);
    for (let i = 0; i < instanceIds.length; i += DB_CHUNK) {
      const chunk = instanceIds.slice(i, i + DB_CHUNK);
  ...
  // ── Phase 5b: Build VCN → subnet → instances metadata ──

  const MAX_INSTANCES_PER_SUBNET = 25;

  // Build subnet OCID → VCN DB ID and subnet OCID → name
  const subOcidToVcnDbId = new Map<string, string>();
  const subOcidToName = new Map<string, string>();
  for (const r of capped) {
    if (r.resourceType === 'network/subnet') {
      const vcnDbId = subnetVcnMap.get(r.id);
      if (vcnDbId) subOcidToVcnDbId.set(r.ocid, vcnDbId);
      subOcidToName.set(r.ocid, r.displayName || r.ocid);
    }
  }

  // Group instances by subnet OCID, count totals, and sample OCIDs for name lookup
  // subnetInstanceCounts: subnet OCID → total instance count
  // subnetInstanceSample: subnet OCID → capped list of instance OCIDs
  const subnetInstanceCounts = new Map<string, number>();
  const subnetInstanceSample = new Map<string, string[]>();

  for (const [instOcid, subOcid] of instanceToSubnetOcid) {
    subnetInstanceCounts.set(subOcid, (subnetInstanceCounts.get(subOcid) || 0) + 1);
    let sample = subnetInstanceSample.get(subOcid);
    if (!sample) { sample = []; subnetInstanceSample.set(subOcid, sample); }
    if (sample.length < MAX_INSTANCES_PER_SUBNET) sample.push(instOcid);
  }

  // Aggregate counts at VCN level
  const vcnInstanceCounts = new Map<string, number>();
  for (const [subOcid, count] of subnetInstanceCounts) {
    const vcnDbId = subOcidToVcnDbId.get(subOcid);
    if (vcnDbId) vcnInstanceCounts.set(vcnDbId, (vcnInstanceCounts.get(vcnDbId) || 0) + count);
  }

  // Load names for all sampled instance OCIDs
  const sampleOcids: string[] = [];
  for (const ocids of subnetInstanceSample.values()) sampleOcids.push(...ocids);
  const instanceMetaMap = new Map<string, { name: string; state: string | null }>();
  for (let i = 0; i < sampleOcids.length; i += DB_CHUNK) {
    const chunk = sampleOcids.slice(i, i + DB_CHUNK);
    const rows = await prisma.resource.findMany({
      where: { snapshotId, ocid: { in: chunk }, resourceType: 'compute/instance' },
      select: { ocid: true, displayName: true, lifecycleState: true },
    });
    for (const row of rows) {
      instanceMetaMap.set(row.ocid, { name: row.displayName || row.ocid, state: row.lifecycleState });
    }
  }

  // Build per-VCN subnet breakdowns
  // vcnSubnetMeta: VCN DB ID → array of { subnetName, subnetOcid, instances[], totalInstances }
  const vcnSubnetMeta = new Map<string, Array<{
    subnetName: string; subnetOcid: string;
    instances: Array<{ name: string; ocid: string; lifecycleState: string | null }>;
  ...
  for (const [subOcid, count] of subnetInstanceCounts) {
    const vcnDbId = subOcidToVcnDbId.get(subOcid);
    if (!vcnDbId) continue;
    let subnets = vcnSubnetMeta.get(vcnDbId);
    if (!subnets) { subnets = []; vcnSubnetMeta.set(vcnDbId, subnets); }
    const sample = subnetInstanceSample.get(subOcid) || [];
    subnets.push({
      subnetName: subOcidToName.get(subOcid) || subOcid,
      subnetOcid: subOcid,
      instances: sample.map(ocid => {
        const meta = instanceMetaMap.get(ocid);
        return { name: meta?.name || ocid, ocid, lifecycleState: meta?.state || null };
      }),
      totalInstances: count,
    });
  }

  // Sort subnets within each VCN by instance count descending
  for (const subnets of vcnSubnetMeta.values()) {
    subnets.sort((a, b) => b.totalInstances - a.totalInstances);
  }
  ...
  if (collapseInstances) {
    ...
    for (const [subDbId, vcnDbId] of subnetVcnMap) {
      const sub = cappedById.get(subDbId);
  ...
  for (const r of capped) {
    ...
    if (rawStr) {
      try {
        ...
        if (r.resourceType === 'compute/instance') {
          if (raw.shape) base.shape = raw.shape;
          // Feature 10: Extract private IP from VNIC attachment data
          const subnetOcid = instanceToSubnetOcid.get(r.ocid);
          if (subnetOcid) {
            // Look for VNIC with IP in attachments
            for (const va of vnicAttachments) {
              if (!va.rawData) continue;
              try {
                const vaRaw = JSON.parse(va.rawData as string);
                if (vaRaw.instanceId === r.ocid && vaRaw.privateIp) {
                  base.privateIp = vaRaw.privateIp;
                  break;
                }
              } catch {}
            }
            // Fallback: derive first usable IP from subnet CIDR
            if (!base.privateIp) {
              const subnet = cappedByOcid.get(subnetOcid);
              if (subnet) {
                const subRawStr = rawDataMap.get(subnet.id);
                if (subRawStr) {
                  try {
                    const subRaw = JSON.parse(subRawStr as string);
                    if (subRaw.cidrBlock) {
                      const cidrBase = subRaw.cidrBlock.split('/')[0];
                      const parts = cidrBase.split('.');
                      if (parts.length === 4) {
                        parts[3] = String(Math.max(2, parseInt(parts[3], 10)));
                        base.privateIp = parts.join('.');
                      }
                    }
                  } catch {}
                }
        ...
        if ((r.resourceType === 'network/load-balancer' || r.resourceType === 'network/network-load-balancer')) {
          ...
          // Feature 10: Extract first IP for LBs
          if (Array.isArray(raw.ipAddresses) && raw.ipAddresses.length > 0) {
            const firstIp = raw.ipAddresses[0]?.ipAddress;
            if (firstIp) base.ipAddress = firstIp;
          }
        }
    ...
    // Attach per-subnet instance breakdown to VCN metadata
    if (r.resourceType === 'network/vcn') {
      const count = vcnInstanceCounts.get(r.id) || 0;
      if (count > 0) {
        metadata = metadata || {};
        metadata.instanceCount = count;
        metadata.subnets = vcnSubnetMeta.get(r.id) || [];
      }
    }
  ...
  if (internetEdges.length > 0) {
    // Feature 5: Populate connected gateway metadata for synthetic nodes
    const internetGateways: Array<{ name: string; type: string }> = [];
    for (const edge of internetEdges) {
      const target = cappedById.get(edge.target);
      if (target) {
        internetGateways.push({
          name: target.displayName ?? target.ocid,
    ...
    nodes.push({
      ...
      lifecycleState: null,
      metadata: {
        connectedGateways: internetGateways,
        connectionCount: internetGateways.length,
      },
      parentNode: null,
    });
  ...
  if (oracleServicesEdges.length > 0) {
    // Feature 5: Populate connected gateway metadata for Oracle Services node
    const oracleGateways: Array<{ name: string; type: string }> = [];
    for (const edge of oracleServicesEdges) {
      const target = cappedById.get(edge.target);
      if (target) {
        oracleGateways.push({
          name: target.displayName ?? target.ocid,
    ...
    nodes.push({
      ...
      lifecycleState: null,
      metadata: {
        connectedGateways: oracleGateways,
        connectionCount: oracleGateways.length,
      },
      parentNode: null,
    });
...
async function buildCompartmentView(
  ...
  // Load compartment resources (always need all compartments for hierarchy)
  const compartments = await prisma.resource.findMany({
    where: { snapshotId, resourceType: 'iam/compartment' },
  ...
  // Aggregate resource counts per compartment — push compartment filter to SQL
  const nonCompWhereClause: Record<string, any> = {
    snapshotId,
    resourceType: { not: 'iam/compartment', notIn: TOPOLOGY_EXCLUDED_TYPES },
  };
  if (relevantCompartmentOcids) {
    nonCompWhereClause.compartmentId = { in: Array.from(relevantCompartmentOcids) };
  }

  // Use groupBy to get counts per compartmentId + resourceType in a single query
  const countRows = await prisma.resource.groupBy({
    by: ['compartmentId', 'resourceType'],
    where: nonCompWhereClause,
    _count: true,
  });

  const countsByCompartment = new Map<string, Record<string, number>>();
  for (const row of countRows) {
    if (!row.compartmentId) continue;
    if (!compartmentOcids.has(row.compartmentId)) continue;
    const compDbId = ocidToDbId.get(row.compartmentId);
    if (!compDbId) continue;
    if (!countsByCompartment.has(compDbId)) {
      countsByCompartment.set(compDbId, {});
    }
    countsByCompartment.get(compDbId)![row.resourceType] = row._count;
  }
  ...
  // Query resource IDs grouped by compartment — avoids loading all resources.
  const resourceIdToCompDbId = new Map<string, string>();
  const compOcidArray = Array.from(compartmentOcids);
  for (let i = 0; i < compOcidArray.length; i += DB_CHUNK) {
    const chunk = compOcidArray.slice(i, i + DB_CHUNK);
    const rows = await prisma.resource.findMany({
      where: { snapshotId, compartmentId: { in: chunk }, resourceType: { not: 'iam/compartment', notIn: TOPOLOGY_EXCLUDED_TYPES } },
  ...
  // Load cross-compartment relationships in chunks
  const trackedIds = Array.from(resourceIdToCompDbId.keys());
  const trackedSet = new Set(trackedIds);
  const REL_CHUNK = DB_CHUNK;
...
async function buildExposureView(
  ...
  // Public IPs — resources with assigned public IPs are internet-reachable
  const publicIps = infra.filter(r => r.resourceType === 'network/public-ip' && r.raw.ipAddress);
  // Map private IP OCID → public IP for linking resources later
  const privateIpToPublicIp = new Map<string, InfraRes>();
  for (const pip of publicIps) {
    if (pip.raw.assignedEntityId) privateIpToPublicIp.set(pip.raw.assignedEntityId, pip);
  }

  // Public subnets — prohibitInternetIngress=false means internet-exposed even if
  // the IGW resource wasn't captured in the snapshot
  const publicSubnets = infra.filter(r =>
    r.resourceType === 'network/subnet' && r.raw.prohibitInternetIngress === false,
  );

  if (activeGateways.length === 0 && publicLbs.length === 0 && publicSubnets.length === 0 && publicIps.length === 0) {
    return { nodes: [], edges: [], totalCount: 0, truncated: false };
  }
  ...
  // Include public subnets (prohibitInternetIngress=false) — these allow internet
  // ingress even if the IGW wasn't captured in the snapshot data
  for (const sub of publicSubnets) {
    if (!internetSubnetOcids.has(sub.ocid)) {
      internetSubnets.push(sub);
      internetSubnetOcids.add(sub.ocid);
    }
  }
  ...
  // Public subnets that aren't already reachable via a gateway edge —
  // connect Internet directly to the subnet to show exposure
  const gatewayLinkedSubnetOcids = new Set<string>();
  for (const gw of activeGateways) {
    // Find subnets routed through this gateway
    for (const sub of internetSubnets) {
      const rtOcid = sub.raw.routeTableId || (sub.raw.vcnId ? vcnDefaultRt.get(sub.raw.vcnId) : null);
      const rules = rtOcid ? routeTableRules.get(rtOcid) : null;
      if (rules?.some(rule => rule.networkEntityId === gw.ocid)) {
        gatewayLinkedSubnetOcids.add(sub.ocid);
      }
    }
  }
  for (const lb of publicLbs) {
    for (const subOcid of (lb.raw.subnetIds || [])) {
      gatewayLinkedSubnetOcids.add(subOcid);
    }
  }

  for (const sub of publicSubnets) {
    if (!gatewayLinkedSubnetOcids.has(sub.ocid)) {
      const subDbId = ocidToDbId.get(sub.ocid);
      if (subDbId) {
        edges.push({
          id: `inet-pubsub-${subDbId}`, source: internetNodeId, target: subDbId,
          label: 'public subnet', relationType: 'internet-edge', animated: true,
        });
      }
    }
  }
  ...
  for (let i = 0; i < idsArray.length; i += DB_CHUNK) {
    ...
    for (const rel of relations) {
      ...
      edges.push({
        ...
        animated: rel.relationType === 'routes-via' || rel.relationType === 'peered-with',
      });
    }
  }
...
// ---------------------------------------------------------------
// Expand collapsed instances for given VCNs or subnets (batched)
// ---------------------------------------------------------------

/**
 * Returns individual instance TopologyNodes for all compute instances
 * within the given VCN/subnet OCIDs. Accepts multiple parents in a
 * single call so VNIC attachments are only loaded once.
 */
export async function expandInstances(
  prisma: PrismaClient,
  snapshotId: string,
  parentOcids: string[],
): Promise<TopologyNode[]> {
  if (parentOcids.length === 0) return [];

  // Resolve parent OCIDs to their types in a single query
  const parents = await prisma.resource.findMany({
    where: { snapshotId, ocid: { in: parentOcids } },
    select: { id: true, ocid: true, resourceType: true },
  });
  if (parents.length === 0) return [];

  // Collect target subnet OCIDs
  const subnetOcids = new Set<string>();
  const vcnOcids: string[] = [];

  for (const p of parents) {
    if (p.resourceType === 'network/subnet') {
      subnetOcids.add(p.ocid);
    } else if (p.resourceType === 'network/vcn') {
      vcnOcids.push(p.ocid);
    }
  }

  // Expand VCNs into their subnets
  if (vcnOcids.length > 0) {
    const vcnSet = new Set(vcnOcids);
    const allSubnets = await prisma.resource.findMany({
      where: { snapshotId, resourceType: 'network/subnet' },
      select: { ocid: true, rawData: true },
    });
    for (const s of allSubnets) {
      ...
      try {
        ...
        if (raw.vcnId && vcnSet.has(raw.vcnId)) subnetOcids.add(s.ocid);
      } catch {}
    }
  }

  if (subnetOcids.size === 0) return [];

  // Single load of VNIC attachments — filter to relevant subnets in JS
  const vnicAttachments = await prisma.resource.findMany({
    where: { snapshotId, resourceType: 'compute/vnic-attachment' },
    select: { rawData: true },
  });
  ...
  const instanceOcids: string[] = [];
  const instanceSubnetOcid = new Map<string, string>();
  for (const va of vnicAttachments) {
    if (!va.rawData) continue;
    try {
      const raw = JSON.parse(va.rawData as string);
      if (raw.instanceId && raw.subnetId && subnetOcids.has(raw.subnetId)) {
        if (!instanceSubnetOcid.has(raw.instanceId)) {
          instanceOcids.push(raw.instanceId);
          instanceSubnetOcid.set(raw.instanceId, raw.subnetId);
        }
      }
    } catch {}
  }

  if (instanceOcids.length === 0) return [];

  // Load instance resources + subnet DB IDs in parallel
  type InstRow = { id: string; ocid: string; displayName: string | null; lifecycleState: string | null; rawData: string | null };
  const [instances, subnetRows] = await Promise.all([
    (async () => {
      const all: InstRow[] = [];
      for (let i = 0; i < instanceOcids.length; i += DB_CHUNK) {
        const chunk = instanceOcids.slice(i, i + DB_CHUNK);
        const batch = await prisma.resource.findMany({
          where: { snapshotId, ocid: { in: chunk }, resourceType: 'compute/instance' },
          select: { id: true, ocid: true, displayName: true, lifecycleState: true, rawData: true },
        });
        all.push(...batch);
      }
      return all;
    ...
    prisma.resource.findMany({
      where: { snapshotId, ocid: { in: Array.from(subnetOcids) } },
      select: { id: true, ocid: true },
    }),
  ]);

  const subnetDbIds = new Map<string, string>();
  for (const row of subnetRows) subnetDbIds.set(row.ocid, row.id);

  return instances.map(inst => {
    let metadata: Record<string, any> | null = null;
    if (inst.rawData) {
      try {
        const raw = JSON.parse(inst.rawData as string);
        const base: Record<string, any> = {};
        if (raw.shape) base.shape = raw.shape;
        metadata = Object.keys(base).length > 0 ? base : null;
      } catch {}
    ...
    const subOcid = instanceSubnetOcid.get(inst.ocid);
    return {
      id: inst.id,
      type: 'instanceNode' as const,
      label: inst.displayName || inst.ocid,
      resourceType: 'compute/instance',
      ocid: inst.ocid,
      lifecycleState: inst.lifecycleState,
      metadata,
      parentNode: subOcid ? (subnetDbIds.get(subOcid) ?? null) : null,
    };
  });
}
