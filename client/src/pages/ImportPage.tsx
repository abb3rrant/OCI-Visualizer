...
export default function ImportPage() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <h2 className="text-2xl font-bold text-gray-900">Import OCI Data</h2>

      {/* Step 1: Download export script */}
      <div className="card">
        <h3 className="text-lg font-semibold mb-2">Step 1: Export from OCI</h3>
        <p className="text-sm text-gray-500 mb-3">Download and run this script to export your OCI environment data.</p>
            ...
            {snapshots.map((s: any) => (
              ...
              >
                <div>
                  ...
                  <div className="text-xs text-gray-500">{s.resourceCount || 0} resources</div>
                </div>
                <span className="text-xs text-gray-400">{new Date(s.importedAt).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
