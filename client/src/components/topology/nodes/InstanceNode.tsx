...
export default React.memo(function InstanceNode({ data }: NodeProps) {
  ...
  if (d?.compact) {
    return (
      <div className="bg-white dark:bg-gray-800 border border-blue-400 dark:border-blue-600 rounded-lg px-2 py-1 flex items-center gap-1.5">
        <span className="w-4 h-4 rounded bg-blue-500 text-white text-[8px] flex items-center justify-center font-bold shrink-0">VM</span>
        <span className="text-[10px] font-medium truncate max-w-[80px] dark:text-gray-200">{d?.label || 'Instance'}</span>
