...
export default function Header({ onToggleSidebar }: HeaderProps) {
  // Show: hamburger menu button, "OCI Visualizer" title, snapshot selector dropdown in center,
  // user email + logout button on right
  // The snapshot selector is a <select> that lists all snapshots from SNAPSHOTS_QUERY
  // When changed, call setCurrentSnapshot with the selected snapshot
  // Style with tailwind: bg-white border-b shadow-sm h-14 flex items-center px-4

  const { user, logout } = useAuth();
  const { currentSnapshot, setCurrentSnapshot } = useSnapshot();
  const [result] = useQuery({ query: SNAPSHOTS_QUERY });
