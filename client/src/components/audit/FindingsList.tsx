...
export default function FindingsList({ findings }: FindingsListProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [findings, filterSeverity, filterCategory, filterFramework, searchTerm]);

  // Reset visible count when filters change
  useEffect(() => { setVisibleCount(50); }, [filterSeverity, filterCategory, filterFramework, searchTerm]);
    ...
    <div className="space-y-4">
      <div className="flex gap-3">
        ...
        <input
          ...
          className="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-700 dark:text-gray-200 flex-1 max-w-xs"
        />
      </div>

      <div className="space-y-2">
        {filtered.slice(0, visibleCount).map((finding, i) => {
                        ...
                        <table className="w-full text-sm">
                          ...
                          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                            {finding.resources.map((r) => (
                              <tr
                                key={r.id}
                                className="bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 cursor-pointer"
                                onClick={() => openResource(r.id)}
                              >
                                <td className="px-3 py-1.5 text-gray-900 dark:text-gray-100">{r.name || '(unnamed)'}</td>
                                <td className="px-3 py-1.5 text-gray-500 dark:text-gray-400 font-mono text-xs max-w-md">
                                  <span className="flex items-center gap-1">
                                    <span className="truncate">{r.ocid}</span>
