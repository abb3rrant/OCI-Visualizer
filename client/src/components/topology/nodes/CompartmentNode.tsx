...
function CompartmentNodeInner({ data }: NodeProps) {
  const d = data as any;
  const metadata = d?.metadata as { resourceCounts?: Record<string, number>; totalResources?: number } | null;
  const label = d?.label || 'Compartment';
  const resourceCounts = metadata?.resourceCounts ?? {};
  const totalResources = metadata?.totalResources ?? 0;

  if (d?.compact) {
    return (
      <div className="bg-amber-50 dark:bg-amber-950 border border-amber-300 dark:border-amber-700 rounded-lg px-2 py-1 flex items-center gap-1.5">
        <svg className="w-3.5 h-3.5 text-amber-600 dark:text-amber-400 shrink-0" fill="currentColor" viewBox="0 0 20 20">
          <path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" />
        </svg>
        <span className="text-[10px] font-medium text-amber-900 dark:text-amber-100 truncate max-w-[80px]">{label}</span>
        {totalResources > 0 && <span className="text-[9px] text-amber-600 dark:text-amber-400">{totalResources}</span>}
