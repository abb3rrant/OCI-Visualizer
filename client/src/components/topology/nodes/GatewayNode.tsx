...
export default React.memo(function GatewayNode({ data }: NodeProps) {
  ...
  if (d?.compact) {
    return (
      <div className="bg-white dark:bg-gray-800 border rounded-lg px-2 py-1 flex items-center gap-1.5" style={{ borderColor: gw.color }}>
        <span className="w-4 h-4 rounded text-white text-[8px] flex items-center justify-center font-bold shrink-0" style={{ backgroundColor: gw.color }}>{gw.short}</span>
        <span className="text-[10px] font-medium truncate max-w-[70px] dark:text-gray-200">{d?.label || 'Gateway'}</span>
