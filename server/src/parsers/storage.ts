...
export function parseMountTargets(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'storage/mount-target',
    displayName: str(item['display-name']),
    compartmentId: str(item['compartment-id']),
    lifecycleState: str(item['lifecycle-state']),
    availabilityDomain: str(item['availability-domain']),
    ...
    rawData: deepCamelCase({
      ...
      exportSetId: item['export-set-id'] ?? null,
      privateIpIds: item['private-ip-ids'] ?? null,
      nsgIds: item['nsg-ids'] ?? null,
    }),
  }));
}

export function parsePreauthRequests(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'storage/preauth-request',
    displayName: str(item['name'] ?? item['display-name']),
    compartmentId: str(item['compartment-id']),
    lifecycleState: null,
    availabilityDomain: null,
    ...
    freeformTags: null,
    rawData: deepCamelCase({
      bucketName: item['bucket-name'] ?? null,
      namespace: item['namespace'] ?? null,
      objectName: item['object-name'] ?? null,
      accessType: item['access-type'] ?? null,
      timeExpires: item['time-expires'] ?? null,
      bucketListingAction: item['bucket-listing-action'] ?? null,
    }),
  }));
}

export function parseLifecyclePolicies(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: `lifecycle-policy://${item['bucket-name'] ?? 'unknown'}`,
    resourceType: 'storage/lifecycle-policy',
    displayName: str(item['bucket-name'] ?? 'Lifecycle Policy'),
    compartmentId: str(item['compartment-id']),
    lifecycleState: null,
    availabilityDomain: null,
    regionKey: null,
    ...
    rawData: deepCamelCase({
      bucketName: item['bucket-name'] ?? null,
      rules: item['items'] ?? item['rules'] ?? null,
    }),
  }));
}

export function parseReplicationPolicies(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'storage/replication-policy',
    displayName: str(item['name'] ?? item['display-name']),
    compartmentId: str(item['compartment-id']),
    lifecycleState: str(item['status'] ?? item['lifecycle-state']),
    availabilityDomain: null,
    regionKey: null,
    timeCreated: str(item['time-created']),
    definedTags: null,
    ...
    rawData: deepCamelCase({
      bucketName: item['bucket-name'] ?? null,
      destinationBucketName: item['destination-bucket-name'] ?? null,
      destinationRegionName: item['destination-region-name'] ?? null,
      timeLastSync: item['time-last-sync'] ?? null,
      status: item['status'] ?? null,
      statusMessage: item['status-message'] ?? null,
    }),
  }));
}
