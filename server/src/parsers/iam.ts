...
export function parseUserGroupMemberships(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'iam/user-group-membership',
    displayName: null,
    compartmentId: str(item['compartment-id']),
    lifecycleState: str(item['lifecycle-state']),
    availabilityDomain: null,
...
export function parseAuthTokens(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'iam/auth-token',
    displayName: str(item['description'] ?? item['display-name']),
    compartmentId: null,
    lifecycleState: str(item['lifecycle-state'] ?? item['state']),
    availabilityDomain: null,
    regionKey: null,
...
export function parseSmtpCredentials(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'iam/smtp-credential',
    displayName: str(item['description'] ?? item['display-name']),
    compartmentId: null,
    lifecycleState: str(item['lifecycle-state'] ?? item['state']),
    availabilityDomain: null,
...
export function parseNetworkSources(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: item['id'] ?? item['ocid'] ?? '',
    resourceType: 'iam/network-source',
    displayName: str(item['name'] ?? item['display-name']),
    compartmentId: str(item['compartment-id']),
    lifecycleState: str(item['lifecycle-state']),
    availabilityDomain: null,
    ...
    rawData: deepCamelCase({
      description: item['description'] ?? null,
      publicSourceList: item['public-source-list'] ?? null,
      virtualSourceList: item['virtual-source-list'] ?? null,
      services: item['services'] ?? null,
    }),
  }));
}

export function parseRegionSubscriptions(json: any): ParsedResource[] {
  return unwrap(json).map((item: any) => ({
    ocid: `region-sub://${item['region-name'] ?? item['region-key'] ?? ''}`,
    resourceType: 'iam/region-subscription',
    displayName: str(item['region-name'] ?? item['region-key']),
    compartmentId: null,
    lifecycleState: str(item['status'] ?? item['lifecycle-state']),
    availabilityDomain: null,
    regionKey: str(item['region-key']),
    timeCreated: null,
    definedTags: null,
    freeformTags: null,
    rawData: deepCamelCase({
      regionName: item['region-name'] ?? null,
      regionKey: item['region-key'] ?? null,
      status: item['status'] ?? null,
      isHomeRegion: item['is-home-region'] ?? null,
    }),
  }));
}
